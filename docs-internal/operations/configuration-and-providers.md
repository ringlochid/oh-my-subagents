...
Operator model discovery does not add a second configuration path. When `workflow_authoring_options` is invoked and Codex is configured, the integration may start a short-lived authenticated Codex app-server client and read its complete visible `model/list` result. This transient catalog is advisory authoring data, is never persisted, and does not establish provider readiness. Failure, timeout, or incomplete pagination yields no catalog and leaves the configured provider default authoritative. Claude remains unavailable through this projection until its integration has an equivalent identity-compatible catalog source.
